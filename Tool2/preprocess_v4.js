...
var state = {
	...
	params: {
		impute: {
			fill_values:  null,   // manual: array of fill values per feature (null = use median)
...
function do_impute(matrix, names, params, mode) {
	...
	var report = { applied: [], medians: [], counts: [] };
	var suggestions = [];
	...
	for (var f = 0; f < n_features; f++) {
		...
		for (var i = 0; i < n_samples; i++) {
			var v = col[i];
			var is_missing = (v === null || v === undefined || isNaN(v));
			if (!is_missing && sentinel !== null && sentinel !== undefined && v === sentinel) {
				is_missing = true;
			}
			if (is_missing) missing_indices.push(i);
			else valid_values.push(v);
		}

		var med = (valid_values.length > 0) ? arr_median(valid_values) : 0;
		report.medians.push(med);
		report.counts.push(missing_indices.length);

		if (missing_indices.length > 0) {
			var pct = (missing_indices.length / n_samples * 100);
			suggestions.push({
				feature:       f,
				name:          names[f],
				missing_count: missing_indices.length,
				missing_pct:   parseFloat(pct.toFixed(1)),
				suggested_fill: med
			});

			// In auto mode: fill immediately. In manual mode: fill only if user provided values.
			var should_fill = (mode === "auto") ||
				(mode === "manual" && params.fill_values && params.fill_values[f] !== undefined);

			if (should_fill) {
				var fill = (mode === "manual" && params.fill_values)
					? params.fill_values[f]
					: med;

				for (var mi = 0; mi < missing_indices.length; mi++) {
					result[missing_indices[mi]][f] = fill;
				}
				report.applied.push({
					feature: f, name: names[f],
					count: missing_indices.length,
					fill_value: fill
				});
			}
		}
...
function commit() {
	...
	for (var i = 0; i < state.n_samples; i++) {
		corpus_out.data[state.sample_keys[i]] = state.working[i].slice();
	}
...
function report() {
	...
	var raw_kept = new Array(state.n_samples);
	for (var i = 0; i < state.n_samples; i++) {
		raw_kept[i] = [];
		for (var k = 0; k < kept.length; k++) {
			raw_kept[i].push(state.raw[i][kept[k]]);
		}
	}
	...
	var sample_size = Math.min(500, state.n_samples);
	var indices = [];
	for (var si = 0; si < state.n_samples; si++) indices.push(si);
	for (var si = 0; si < sample_size; si++) {
		var sj = si + Math.floor(Math.random() * (state.n_samples - si));
